...
def sm_ridge(X, Y, alpha=0.005):
    ...
    for train_index, test_index in ts.split(X):
        ...
        # reg_results = model.fit_regularized(alpha = alpha, L1_wt=0.0)
...
def sm_plot_actual_v_predicted(fig, fitted_model, X, Y_actual):
	fig.scatter(fitted_model.predict(X) / 1000000, Y_actual / 1000000, alpha=.3)
...
def plot_actual_v_predicted(fitted_model, X, Y_actual):
	f = bokeh.plotting.figure()
	f.scatter(fitted_model.best_estimator_.predict(X), Y_actual)
	min_max = Y_actual.min(), Y_actual.max()
	actual_range = zip(min_max, min_max)
	f.line(min_max, min_max, color='black')
	
	bokeh.io.show(f)

def plot_resid_v_actual(fitted_model, X, Y_actual):
	f = bokeh.plotting.figure()
	res = Y_actual - fitted_model.best_estimator_.predict(X)
	f.scatter(Y_actual, res)
	bokeh.io.show(f)
