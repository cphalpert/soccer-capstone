...
library(tidyverse)
give.n <- function(x){
  return(c(y = -5000000, label = length(x))) 
}
...
```{r}
ggplot(df, aes(x=market_val_prev, y=market_val)) + geom_point(alpha=0.2) + 
  scale_y_continuous(labels = function(x) format(x/1000000)) +
...
df_season <- df2 %>% group_by(team) %>% summarise(max_season = max(num_of_seasons)) 
df2 <- merge(x=df2, y=df_season, by="team", all.x=TRUE)
df2$team_season <- paste(df2$team, "(", df2$max_season, ")")
...
```{r}
#df2$team_season <- factor(df2$team_season, levels=df2$team_season[order(df2$avg_mkt_val_per_season)])

ggplot(df2, aes(y=avg_mkt_val_per_season, x=reorder(team_season, avg_mkt_val_per_season, sum), fill=pos_type)) +
coord_flip() + geom_bar(stat="identity") + 
scale_y_continuous(labels = function(x) format(x/1000000)) + 
ggtitle("Figure 2. Barchart of Market Value By Team and Position") +
xlab("team (number of seasons)") + ylab("average market value per season (pounds, millions)")
```
...
grid.arrange(p1, p2, p3, p4, ncol=2, top = "Figure 3. Density Plot of Performance by Position")
```
